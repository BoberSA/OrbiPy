...
def newton_int(propCrtbp, event, t0, s0, mu1, int_param, tol=1e-12, maxiter=50, debug=False):
...
def brent(propCrtbp, event, t0, t1, s0, mu1, int_param, tol=1e-12, maxiter=50, debug=False):
    #from crtbp_prop import propCrtbp
...
def correctEvents(propCrtbp, events, evout, prop, tol=1e-12, sn=6, maxiter=50, **kwargs):
    '''
...
#    atol = kwargs['int_param']['atol']
    ...
    for ev in evout[1:]:
        ...
        ret = newton_int(propCrtbp, events[ev[0]], 
                         ev[2][sn], 
                         ev[2][:sn], 
                         tol=tol,
                         maxiter=maxiter,
        ...
        if ret is None:
            ...
            t, s = brent(propCrtbp, events[ev[0]], 
                         ev[2][sn], 
                         ev[3][sn], 
                         ev[2][:sn], 
                         tol=tol,
